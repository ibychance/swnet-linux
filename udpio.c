...
int __udp_rx(ncb_t *ncb) {
    ...
    if (0 == recvcb) {
        nis_call_ecr("nshost.udpio.rx: link %lld zero bytes return by syscall recvfrom", ncb->hld);
        return -1;
    }
    ...
    if (recvcb < 0){
        ...
        nis_call_ecr("nshost.udpio.rx: link %lld syscall recvfrom error, code:%d", ncb->hld, errcode);
        return -1;
    }
