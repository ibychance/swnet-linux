...
int __tcp_syn(ncb_t *ncb_server)
{
    ...
    if (tcp_save_info(ncb_server, &ktcp) >= 0) {
        if (ktcp.tcpi_state != TCP_LISTEN) {
            mxx_call_ecr("state illegal,link:%lld, kernel states %s.", ncb_server->hld, tcp_state2name(ktcp.tcpi_state));
            return 0;
        }
    }
...
int tcp_txn(ncb_t *ncb, void *p)
{
    ...
    while (node->offset < node->wcb) {
        ...
        if (wcb < 0) {
            ...
            if (EAGAIN == errno) {
                mxx_call_ecr("syscall send(2) would block cause by kernel memory overload,link:%lld", ncb->hld);
                return -EAGAIN;
            }
...
int tcp_rx_syn(ncb_t *ncb)
{
    ...
    if (0 == (retval = getsockopt(ncb->sockfd, SOL_SOCKET, SO_ERROR, &error, &errlen))) {
        ...
        mxx_call_ecr("fatal error occurred syscall getsockopt(2), error:%d,link:%lld", errno, ncb->hld);
    }
